...
@Component
public class VoiceMessageProcessor implements MessageTypeProcessor {
    @Override public MessageType supportedType() { return MessageType.VOICE; }
    @Override public void validate(SendMessageRequest request) {
        if (StringUtils.isBlank(request.getMediaUrl()))
            throw new BusinessException("语音地址不能为空");
    }
    @Override public String buildPreview(ChatMessage message) { return "[语音]"; }
}
