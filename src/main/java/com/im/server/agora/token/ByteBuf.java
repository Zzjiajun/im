...
public class ByteBuf {
    ...
    public ByteBuf put(TreeMap<?, ?> extra) {
        put((short) extra.size());
        for (Map.Entry<?, ?> pair : extra.entrySet()) {
            put((Short) pair.getKey());
            put(String.valueOf(pair.getValue()));
        }
        return this;
    }

    public ByteBuf putIntMap(TreeMap<?, ?> extra) {
        put((short) extra.size());
        for (Map.Entry<?, ?> pair : extra.entrySet()) {
            put((Short) pair.getKey());
            put((Integer) pair.getValue());
        }
        return this;
    }
